...
app.get("/jokes/:id", (req, res) => {
  ...
  const specificJoke = jokes.find((item) => item.id === id);
...
app.get("/filter", (req, res) => {
  ...
  const jokesFiltered = jokes.filter((item) => item.jokeType === type);
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const text = req.body.text;
  const type = req.body.type;
  const oldJoke = jokes.find((item) => item.id === id);
  const changedJoke = {
    id: id,
...
//9. (put by me) To get all jokes as base data
app.get("/", (req, res) => {
  res.json(jokes);
});

app.set("json spaces", 2); //to make it pretty and give intendation=2
